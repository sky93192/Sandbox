...
/** 收到表單後動作 */
	...
	if(empty($_POST["new_publisher"])){
		...
		$publisher = mysql_real_escape_string(strval($_POST["new_publisher"]));
	}

	if(empty($_POST["new_book_name"])){
		$errors[] = '請填寫書名';
	}else{
		$book_name = mysql_real_escape_string(strval($_POST["new_book_name"]));
	}

	if(empty($_POST["new_author"])){
		$errors[] = '請填寫作者';
	}else{
		$author = mysql_real_escape_string(strval($_POST["new_author"]));
	}
	...
	if(!isset($_POST["new_price"])){
		...
		if (!preg_match("/^[0-9]+$/",$_POST["new_price"])) {
			...
			$price = mysql_real_escape_string(strval($_POST["new_price"]));
		}
